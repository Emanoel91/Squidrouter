...
total_volume = volume_sorted.sum()

# ---------- User Concentration (Top 1%) ----------

top_1_percent = max(1, int(np.ceil(len(user_df) * 0.01)))

user_concentration = (
    volume_sorted.iloc[:top_1_percent].sum()
    / total_volume
    * 100
)
...
values = np.sort(user_df["volume"].values)
...
if n == 0 or values.sum() == 0:
    ...
    gini = (
        np.sum((2 * np.arange(1, n + 1) - n - 1) * values)
        / (n * values.sum())
    )
...
whale_users = (
    user_df["volume"] >= WHALE_THRESHOLD
).sum()
...
power_users = (
    user_df["num_txs"] >= POWER_THRESHOLD
).sum()
...
with k1:
    st.metric(
        label="User Concentration",
        value=f"{user_concentration:.2f}%",
        help="""
Percentage of total cross-chain volume generated by the top 1% of users.

Higher values indicate that a small number of users account for a large share of activity.
"""
    )
...
with k2:
    st.metric(
        ...
        value=f"{gini:.3f}",
        help="""
Measures how evenly transaction volume is distributed across users.

• 0 = perfectly equal distribution

• 1 = one user generates nearly all volume
"""
    )

with k3:
    st.metric(
        label="Top 10 Users Share",
        value=f"{top10_share:.2f}%"
    )
...
with k4:
    st.metric(
        ...
        value=f"{whale_users:,}",
        help=f"""
Number of users whose cumulative cross-chain volume exceeds ${WHALE_THRESHOLD:,.0f}.
"""
    )
